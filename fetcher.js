// Import file system, request and readline. 
...
// EDGE CASE: If the path exists, initiate a readline, if y + enter, then execute rest of code, otherwise end.
//            If the path doesn't exist, just execute the code. 
// NOTE: fs.access only returns an error if the file does NOT exist. So if no error, then the file exists!
fs.access(path, fs.constants.F_OK, (err) => {
  if (!err) {
    console.log(`File exists!`);
    rl.question(`Would you like to overwrite the existing file? Press y, followed by enter for yes. \n> `, answer => {
      if (answer === 'y') {
        // It should request the resource at the URL
        request(url, (error, response, body) => {
          ...
          // Write to the local path on your machine
